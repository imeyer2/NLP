...
class QuasiNewton(Optimizer):
    ...
    def __init__(
        ...
        # self.solver = Solver(max_krylov, krylov_tol) #don't need this test?
    ...
    def _get_flat_grad(self) -> Tensor: #(ian.meyer@pnnl.gov)'s comment: this entire function was from here: #https://discuss.pytorch.org/t/vector-view-of-model-parameters/3783
        views = []
        for p in self._params:
            # print(p)
            # print(p.data)
            # print(p.data.numel())
            if p.grad is None:
                view = p.data.new(p.data.numel()).zero_() #make a bunch of zeros as gradient #TODO: Pull request this...
                # view = torch.zeros_like(input = p).view(-1)   #OLD line
            elif p.grad.data.is_sparse:
                view = p.grad.data.to_dense().view(-1)
            else:
                view = p.grad.data.view(-1)
