...
class Yahoo_Answers_Dataset(object):
    ...
    def __init__(self, dict_size=10000, valid_ratio=0.1, max_len=100,
        ...
        if self.__debug:
            s = time.time()
        self.test = {"seq": None, "label": None}
        f = gzip.open(test_path, "rb")
        d = pickle.load(f)
        self.test["seq"] = d["text"]
        self.test["label"] = d["label"]
        f.close()
        if self.__debug:
            e = time.time()
            print ("Test-set loaded!")
            print ("\tTest# = %d" % len(self.test["label"]))
            print ("\tTime cost = %.3f sec" % (e-s))

        # load & split train/valid-set
        if self.__debug:
            s = time.time()
        self.train = {"seq": None, "label": None}
        ...
        self.train["seq"] = d["text"][split_idx:]
        self.train["label"] = d["label"][split_idx:]
        f.close()
        if self.__debug:
            e = time.time()
            print ("Train-set (along with valid-set) loaded!")
            print ("\tTrain# = %d" % len(self.train["label"]))
        ...
        if self.__debug:
            s = time.time()
        self.__av = random.sample(range(len(self.train["label"])),
                                  len(self.train["label"]))
        if self.__debug:
    ...
    def minibatch(self, batch_size):
        ...
        if batch_size > len(self.train["label"]):
            batch_size = len(self.train["label"])
            if self.__debug:
        ...
        for idx in chosen_idx:
            # get length
            batch[2].append(len(self.train["seq"][idx]))
            # get label
            batch[1].append(self.train["label"][idx]-1)   # 1-index to 0-index
            # get sequence
            tmp_seq = [0 for i in range(self.__max_len)]
            for seq_idx in range(len(self.train["seq"][idx])):
                tmp_seq[seq_idx] = self.__dictionary.get(self.train["seq"][idx][seq_idx],
                                                         self.__dict_size)
    ...
    def test_batch(self, batch_size):
        ...
        if batch_size > len(self.test["label"]):
            batch_size = len(self.test["label"])
            if self.__debug:
                print ("Batch size larger than test#!")
        # randomly generate the index list
        chosen_idx = random.sample(range(len(self.test["label"])), batch_size)
        batch = [[] ,[], []]
        
        for idx in chosen_idx:
            # get length
            batch[2].append(len(self.test["seq"][idx]))
            # get label
            batch[1].append(self.test["label"][idx]-1)   # 1-index to 0-index
            # get sequence
            tmp_seq = [0 for i in range(self.__max_len)]
            for seq_idx in range(len(self.test["seq"][idx])):
                tmp_seq[seq_idx] = self.__dictionary.get(self.test["seq"][idx][seq_idx],
                                                         self.__dict_size)
            batch[0].append(tmp_seq)
